...
class Player:
    def __init__(self, data):
        self.name = data["name"]
        self.nationality = str(data["nationality"]).upper()
...
class PlayerReader:
    ...
    def get(self) -> List[Player]:
        request = get(self.url, timeout=60)
        if request.status_code != 200:
            raise ConnectionError(f"Failed to retrieve stats ({request.status_code})")
...
class PlayerStats:
    ...
    def get_available_nationalities(self) -> List[str]:
        return list({player.nationality for player in self.player_reader.get()})
